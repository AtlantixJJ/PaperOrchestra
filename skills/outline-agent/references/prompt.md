...
Your inputs are:
  ...
  2. experiments/: A folder of .md files containing experimental results,
     including raw data points, ablation studies, and performance metrics.
     Read all files filename-sorted and concatenate before processing.
...
Synthesize experiments/ and idea.md to identify the most compelling
evidence.
  ...
  - The data_source MUST be exactly "idea.md", "experiments/", or
    "both".
...
Introduction Strategy (Macro-Level Context, 10-20 papers):
  ...
  - Hypotheses: Define the "Hook" (broad context) and "Problem Gap" to be
    ...
    the specific datasets and evaluations present in experiments/.
...
Conclusion, Appendix) into a detailed structural plan.
  ...
  - Mandatory Citations (citation_hints): You must provide targeted citation
    ...
    - Required Coverage (EXHAUSTIVE): You MUST explicitly create a targeted
      ...
      LLaMA, GPT, LLaVA). If it is in the experiments/ or idea.md,
...
Guidelines on Scientific Depth & Mathematical Rigor:
  ...
  - Grounded Formalization: Propose explicit subsections for rigorous
    ...
    experiments/; do not instruct the writing agent to include
